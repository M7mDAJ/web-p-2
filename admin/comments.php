...
if (isset($_POST['delete_comment'])) {
    ...
    $verify_comment = $conn->prepare("SELECT id FROM `comments` WHERE id = ?");
...
<section class="comments">
    ...
    <div class="show-comments">
        <?php
        $select_comments = $conn->prepare("SELECT id, content_id, tutor_id, comment, date FROM `comments` WHERE tutor_id = ?");
        $select_comments->execute([$tutor_id]);

        if ($select_comments->rowCount() > 0) {
            while ($fetch_comment = $select_comments->fetch(PDO::FETCH_ASSOC)) {
                $select_content = $conn->prepare("SELECT id, title FROM `content` WHERE id = ?");
